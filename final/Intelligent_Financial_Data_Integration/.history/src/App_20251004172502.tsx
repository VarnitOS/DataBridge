...
export default function App() {
  ...
  const stepOrder: Step[] = ["upload", "mapping", "conflicts", "results"];
  ...
  const canGoNext = () => {
    switch (currentStep) {
      ...
      case "conflicts":
        return true; // Conflicts step is always valid
      case "results":
  ...
  const handleNextStep = () => {
    const currentIndex = getCurrentStepIndex();
    if (currentIndex < stepOrder.length - 1 && canGoNext()) {
      setCurrentStep(stepOrder[currentIndex + 1]);
    }
  };
