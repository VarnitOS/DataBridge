...
export default function App() {
  ...
  const handleCloseTab = (tabId: string) => {
    ...
    // If the closed tab was active, switch to integration view
  ...
  const addFinancialInstitution = () => {
    ...
    setFinancialInstitutions(prev => [...prev, {
      id: newId,
      label: newLabel,
      file: null,
      data: []
  ...
  const updateFinancialInstitutionFile = (id: string, file: File | null) => {
    setFinancialInstitutions(prev => prev.map(fi => 
      fi.id === id ? { ...fi, file } : fi
    ));
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Update financial institutions with parsed data
      setFinancialInstitutions(prev => prev.map(fi => {
        const parsed = parsedData.find(p => p.id === fi.id);
        return parsed ? { ...fi, data: parsed.data } : fi;
      }));

      // For backward compatibility, set source and target data from first two files
      if (parsedData.length >= 2) {
        setSourceData(parsedData[0].data);
        setTargetData(parsedData[1].data);
      }

      // Generate mappings using the first two datasets
                      ...
                      <ParticleCard className="card card--border-glow" glowColor="132, 0, 255" particleCount={12} enableTilt enableMagnetism clickEffect>
                        <Card className="p-6">
                          <div className="flex items-center justify-between mb-6">
                            <h2>Upload Financial Institution Datasets</h2>
                          ...
                          <Alert className="mb-6">
                            ...
                            <AlertDescription>
                              Upload CSV files from multiple financial institutions. Our AI will automatically analyze and map the schemas.
                            </AlertDescription>
                          </Alert>
                          <div className="space-y-6 mb-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[95vh] overflow-y-auto pr-2">
                              ...
                              <div className="flex items-center justify-center">
                                <Button
                                  ...
                                  variant="outline"
                                  className="w-full h-32 border-2 border-dashed border-primary/50 hover:border-primary hover:bg-primary/5 transition-colors"
                                  disabled={status === "processing"}
                                >
                                  <div className="flex flex-col items-center gap-2">
                                    ...
                                    <span className="text-sm text-muted-foreground">Add Financial Institution</span>
                                  </div>
                                </Button>
                              </div>
                            </div>
