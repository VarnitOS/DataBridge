    ...
    <Card className="p-6 bg-gradient-to-br from-card to-muted/20">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between p-4 bg-background/50 rounded-lg">
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className={`w-4 h-4 rounded-full ${config.color} ${status === 'processing' ? 'animate-pulse' : ''}`}></div>
              <div className={`absolute inset-0 w-4 h-4 rounded-full ${config.color} ${config.glow} blur-sm ${status === 'processing' ? 'animate-pulse' : ''}`}></div>
            </div>
