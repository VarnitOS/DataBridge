...
export type PillNavItem = {
  label: string;
  href?: string;           // optional – not used when onClick is provided
  ariaLabel?: string;
  onClick?: () => void;    // callback-based nav (no router required)
  onClose?: () => void;    // callback for closing the tab
  closable?: boolean;      // whether the tab can be closed (default: false)
};

export interface PillNavProps {
  items: PillNavItem[];
  activeLabel?: string;    // which item is active (by label)
  className?: string;
  ease?: string;
  baseColor?: string;
  pillColor?: string;
  ...
  onAddTab?: () => void;   // callback for adding new tab
}
            ...
            {items.map((item, i) => (
              <li key={item.label} role="none">
                <button
                  type="button"
                  role="menuitem"
                  className={`pill${activeLabel === item.label ? ' is-active' : ''}`}
            ...
            {onAddTab && (
              <li role="none">
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = `${baseColor}20`;
        ...
        <ul className="mobile-menu-list">
          {items.map(item => (
            <li key={item.label}>
              <button
                type="button"
                className={`mobile-menu-link${activeLabel === item.label ? ' is-active' : ''}`}
                onClick={() => {
                  item.onClick?.();
